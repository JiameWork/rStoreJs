...
## 说明：
通过了解rstore众多的API，您应该可以发现rstore为多rstore实例并行预留来到很多接口，从creatRStore初始化对象推荐填写的rstore名称或creatProxy中的第二参数为数组格式的仓库名称绑定，您都可以发现rstore运行并行的身影，您可以在下方的启发演示中熟悉多rstore实例并行的使用和它的优点。

!>注意此部分具有一定的复杂性，推进您熟悉基础API后阅读。
...
const store = {
    ...
    actions(rstoreData) {
        ...
        return {
            reqUserData, reqBannerList, reqPushAddress,setSumTitel
        }
    }
}
...
busStore.js 如果仅仅把rstore当作总线使用那么rstore实例会非常的轻便
...
class App extends Component {
  ...
  componentDidMount() {
    ...
    firstStore.setSumTitel('1234567891235', '这里是副标题，它的长度肯定要超过16个字符了') //触发actions中的预设方法
...
export default class App extends Component {
  render() {
    const { data: { titel, miniTitel } } = firststore //使用firstStore中的响应式数据
    return (
      <div>
        <h1>{titel}</h1>
        <h2>{miniTitel}</h2>
  ...
  componentDidMount() {
    ...
    firststore.actions.setSumTitel('1234567891235', '这里是副标题，它的长度肯定要超过16个字符了') //触发actions中的预设方法
  }
}
